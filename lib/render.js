...
// This maps data object to active nodes.
var activeNodesMap = new WeakMap()

// Option to use comment nodes as markers.
Object.defineProperty(render, 'useCommentNode', {
...
function reconcile (scope, obj, def, parentNode, path, state) {
  ...
  for (i = 0, j = defKeys.length; i < j; i++) {
    ...
    activeNodes = activeNodesMap.get(obj)
    if (!activeNodes) {
      activeNodes = {}
      activeNodesMap.set(obj, activeNodes)
    }
    if (!activeNodes[key]) activeNodes[key] = []
    activeNodes = activeNodes[key]
    ...
    if (branch[isBoundToParentKey]) {
      value = state[key]
      target = obj[key]
      state[key] = target != null ? memoize(target) : target
    ...
    for (k = 0, l = Math.max(value.length, target.length); k < l; k++) {
      ...
      // Check for removal.
      if (b == null && a != null) {
        currentNode = activeNodes[k]

        if (change)
          returnValue = change(currentNode, null, a, endPath)
          ...
          returnValue = mount(currentNode, null, endPath)
        }

        if (returnValue !== simulacra.retainElement)
          marker.parentNode.removeChild(currentNode)
      ...
      // Check for update.
        ...
        if (definition) {
          ...
          reconcile(scope, b, definition, currentNode, endPath, a)
        }
        else {
          if (a === b) continue

          returnValue = change ? change(currentNode, b, null, endPath) : b
    ...
    if (target.length !== activeNodes.length)
      activeNodes.length = target.length

    for (k = 0, l = target.length; k < l; k++)
      if (typeof target[k] === 'object')
        target[k] = memoize(target[k])
...
function memoize (obj) {
  ...
  for (i = 0, j = keys.length; i < j; i++) {
    key = keys[i]
    memo[key] = Array.isArray(obj[key]) ? obj[key].slice(0) : [ obj[key] ]
  }
