...
// KNOWN BUG: incorrectly modifies last index with newly inserted argument.
function patchUnshift (scope, obj, def, key) {
  ...
  return function unshift () {
    ...
    for (j = 0, k = arguments.length; j < k; j++) {
      currentNode = replaceNode(scope, obj, def, key, arguments[j], null, j)
      marker = meta.currentMarker
      if (currentNode)
        if (isMarkerLast) {
          marker.parentNode.appendChild(currentNode)
...
// KNOWN BUG: insertion order of arguments is reversed.
